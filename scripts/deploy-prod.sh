# This script is intended for local / VM-based deployments
# Paths and permissions may need adjustment based on environment
services:
  prometheus:
    image: prom/prometheus:latest
    container_name: prometheus_stage
    ports:
      - "9090:9090"
    volumes:
  ...
  node_exporter:
    ...
    container_name: node_exporter_stage
    ports:
      - "9100:9100"
    restart: unless-stopped
abhishekm@DESKTOP-TMGOHEI:~/monitoring-poc/stage$ cd ..
abhishekm@DESKTOP-TMGOHEI:~/monitoring-poc$ cd prod/
abhishekm@DESKTOP-TMGOHEI:~/monitoring-poc/prod$ ls
docker-compose.yml  grafana-data  prometheus  prometheus-data
abhishekm@DESKTOP-TMGOHEI:~/monitoring-poc/prod$ cat docker-compose.yml
services:
  prometheus:
    image: prom/prometheus:latest
...
abhishekm@DESKTOP-TMGOHEI:~/monitoring-poc/prod$ ls
docker-compose.yml  grafana-data  prometheus  prometheus-data
abhishekm@DESKTOP-TMGOHEI:~/monitoring-poc/prod$ cd ~
abhishekm@DESKTOP-TMGOHEI:~$ ls
deploy_prod.sh  monitoring-poc  snap  ubuntu
abhishekm@DESKTOP-TMGOHEI:~$ cat deploy_prod.sh
#!/bin/bash
set -e
